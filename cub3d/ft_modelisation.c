...
int    ft_find_wall(t_glob *glob, float ty, int sinry, int cosrx)
{
    float   tx;
    
    tx = (int)(((int)glob->rx + (int)glob->ry) / (SQR / RES)) % RES;
    if (glob->map[(int)((glob->ry - 1) / SQR)][(int)(glob->rx / SQR)] != '1'
        && (int)((glob->ry - 1) / SQR) == (int)((sinry) / SQR))
        return(glob->n_img->data[(int)tx + (int)ty * RES]);
    else if (glob->map[(int)((glob->ry + 1) / SQR)][(int)(glob->rx / SQR)] != '1'
        && (int)((glob->ry + 1) / SQR) == (int)((sinry) / SQR))
        return(glob->s_img->data[(int)tx + (int)ty * RES]);
    else if (glob->map[(int)(glob->ry / SQR)][(int)((glob->rx + 1) / SQR)] != '1'
        && (int)((glob->rx + 1) / SQR) == (int)((cosrx) / SQR))
    ...
    {
        if ((glob->ra <= PI / 4 && glob->ra >= 0) || (glob->ra >= 7 * PI / 4
            && glob->ra <= 2 * PI))
            return(glob->w_img->data[(int)tx + (int)ty * RES]);
            ...
            return(glob->e_img->data[(int)tx + (int)ty * RES]);
        else if (glob->ra >= PI / 4 && glob->ra <= 3 * PI / 4)
            return(glob->n_img->data[(int)tx + (int)ty * RES]);
    }
    return (0);
}
...
void    ft_modelisation(t_glob *glob, int i)
{
    ...
    while (++y < lineh)
    {
        glob->data[(int)(i + (y + lineo) * SCREEN_W)]
            = ft_find_wall(glob, ty, glob->ry - sin(glob->ra), glob->rx - cos(glob->ra));
        ty += ty_step;
    }
    y = -1;
    while (++y < lineo)
...
void    ft_ray_maker(t_glob *glob, int i)
{
    ...
    while (++i < SCREEN_W)
    {
        ...
        cosra = cos(glob->ra);
        sinra = sin(glob->ra);
        while (glob->map[(int)(glob->ry / SQR)][(int)(glob->rx / SQR)] != '1' && glob->map[(int)(glob->ry / SQR)][(int)(glob->rx / SQR)] != '\0' && glob->map[(int)(glob->ry / SQR)][(int)(glob->rx / SQR)] != ' ')
        {
            glob->rx += cosra;
            glob->ry += sinra;
            glob->length++;
